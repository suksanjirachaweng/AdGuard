...
export default function Topbar({ onMenuToggle }) {
  ...
  return (
    <header className="topbar" style={st("height:62px;flex-shrink:0;background:#fff;border-bottom:1px solid #e2e9e5;display:flex;align-items:center;padding:0 16px;gap:14px;")}>
      <button
        onClick={onMenuToggle}
        title="เมนู"
        style={st("background:none;border:none;cursor:pointer;padding:6px;display:flex;flex-direction:column;gap:5px;flex-shrink:0;")}
      >
        <span style={st("display:block;width:22px;height:2px;background:#157347;border-radius:2px;")} />
        <span style={st("display:block;width:22px;height:2px;background:#157347;border-radius:2px;")} />
        <span style={st("display:block;width:22px;height:2px;background:#157347;border-radius:2px;")} />
      </button>
